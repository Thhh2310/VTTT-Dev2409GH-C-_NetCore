...
namespace Session09.Controllers
{
    public class CustomerMemberController : Controller
    {
        ...
        {
            var pass = model.Password;
            var data = _context.Customers.Where(x=>x.Isactive==1).Where(x=>x.Usename.Equals(model.UserOrEmail) || x.Email.Equals(model.UserOrEmail)).FirstOrDefault(x=>x.Password.Equals(pass));
